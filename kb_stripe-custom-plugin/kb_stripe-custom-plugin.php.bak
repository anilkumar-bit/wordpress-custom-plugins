    ...
    if ($this->checkRecurring()) {
        ...
        if ($pid) {
			...
			if (is_user_logged_in()) {
				...
				$new_user_email = !empty($_POST["emailField"]) ? $_POST["emailField"]: 'newuser@example.com'; 
				$new_user_password = 'SecurePassword123'; // Replace with a secure password
				$new_user_name = !empty($_POST["firstNameField"]) ? $_POST["firstNameField"]: 'New User'; 
				...
				if ($user_id) {
					...
					$user_name = !empty($existing_user->display_name) ? $existing_user->display_name : '';
				} else {
					 $user_id = wp_create_user($new_user_name, $new_user_password, $new_user_email);
					 // Optionally log the new user in
            ...
            if ($response['success']) {
                ...
                if ($order_exists) {
                    ...
                    $commission_table = $wpdb->prefix . "Commission_data";
                    $transferlog_table = $wpdb->prefix . "transferlog_table";
                    $transfer_exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $commission_table WHERE order_id = %d", $order_id));
                    ...
                    $percent = get_option("kb_conncted_account_commission");
                ...
                } // if order exists closed
            } else {
                echo json_encode(['error' => 'Subscription creation failed','resp '=>$response]);
				exit;
            }
        }
    } else {
	   ...
	   global $wpdb;
		...
		try {
			...
			if (count($existing_customers->data) > 0) {
				...
				//\Stripe\PaymentMethod::attach($paymentMethod->id, ['customer' => $customer->id]);
			...
			// Convert Stripe price to GBP (assuming it's in pence)
			$product_amount = $price->unit_amount / 100; // 4.95 GBP
			$platform_commission = get_option("kb_conncted_account_commission"); // 20%
			// Add Stripe fee (2.5%) to platform commission
			$stripe_fee_percent = 2.5; // 
			$percentage_deducted = (0.20 /100 ) * $product_amount;
			$total_application_fee_percent = round($platform_commission + $stripe_fee_percent +$percentage_deducted, 2);
			
			/* $resss = [
				'platform_commission' => $platform_commission,
				'stripe_fee_percent' => $stripe_fee_percent,
				'percentage_deducted' => $percentage_deducted,
				'customer' => $customer->id,
				'items' => [
					['price' => $stripe_price_id], 
				],
				'default_payment_method' => $paymentMethod->id, 
				'application_fee_percent' => $total_application_fee_percent, 
				'transfer_data' => [
					'destination' => $connected_account_id, 
				],
			];
			...
			die;
			 */
			 $transfer_data = ($platform_commission < 100) ? 'yes' : 'no'; 
			 if($transfer_data =='no'){
				 // Create the subscription (all payments stay in the main platform account)
				$subscription = \Stripe\Subscription::create([
				 ...
				 // Create the subscription with transfer logic
				$subscription = \Stripe\Subscription::create([
					'customer' => $customer->id,
					'items' => [
						['price' => $stripe_price_id], 
		    ...
		    //$response['product_amount'] = $product_amount;
